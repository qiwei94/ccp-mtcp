...
typedef struct tcp_stream
{
	...
	uint16_t on_timeout_list:1, 
            ...
            wait_for_acks:1;        /* if true, the sender should wait for acks to catch up */
	
	uint32_t snd_nxt;		/* send next */
	uint32_t rcv_nxt;		/* receive next */
    uint32_t seq_at_last_loss;      /* the sequence number we left off at before we stopped due to loss */ 
